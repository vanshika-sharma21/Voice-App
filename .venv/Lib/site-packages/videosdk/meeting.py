...
class Meeting:
    ...
    def switch_to(
        ...
        """Switch to a new meeting."""
        logger.debug(f"switching to meeting {meeting_id}")
        self.__room_client.switch_to(
            meeting_id=meeting_id,
            local_participant_id=self.__local_participant.id,
        )
    ...
    def __listen(self):
        ...
        def on_participant_joined(data):
            ...
            peer_type = data.get("peerType", "normal")

            is_agent = peer_type == "agent"
            ParticipantClass = Agent if is_agent else Participant
            ...
            if is_agent:
                self.__emit(VideoSDKEvents.EV_AGENT_JOINED.value, participant)
            else:
        ...
        def on_participant_left(data):
            ...
            if id in self.__participants:
                ...
                if isinstance(participant, Agent):
                    self.__emit(VideoSDKEvents.EV_AGENT_LEFT.value, participant)
                else:
        ...
        @self.__room_client.on(Events.SWITCH_ROOM)
        def on_switch_room(data):
                for peer_id in list(self.__participants.keys()):
                    if peer_id != self.__local_participant.id:
                        if peer_id in self.__participants:
                            participant = self.__participants.pop(peer_id)
                            self.__emit(VideoSDKEvents.EV_PARTICIPANT_LEFT.value, participant)
                            participant.remove_all_listeners()
                            participant._cleanup()
                            del participant
